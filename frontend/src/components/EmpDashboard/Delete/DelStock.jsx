...
export default function DelStock({id}) {

  const handleDelete = async () => {
    try {
      await axios.delete(`http://192.168.0.13:8080/stocks/del/${id}`,
        {
          headers: {
            'Authorization': token()
          }
  ...
  return (
    <AlertDialog>
      ...
      <AlertDialogContent>
          ...
          <AlertDialogDescription>
          Cette action vas supprimer la stock. Voulez vous vraiment supprimer le stock ?
          </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
